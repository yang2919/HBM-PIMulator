...
class System(Memory):
    ...
    def PIM_GEMV_BO(self, in_bo1: Buffer, in_bo2: Buffer, out_bo: Buffer, op_trace: bool):
        ...
        while idx_cur_col < num_cols_per_bank:
            ...
            interval_num = max(1, in_bo1.size // num_rfs)

            for iter in range(interval_num):
                iter_index = interval_size * iter + idx_cur_col * in_bo1.size
                num_cur_rfs = min(num_rfs, in_bo1.size - num_rfs * iter)
                if (interval_num == 1 and iter == 0) or interval_num > 1:
                ...
                for rf_w in range(num_rfs_out):
                    for rf_x in range(num_cur_rfs):
                        row, col = in_bo2.get_index(self.DRAM_column, iter_index + rf_w * num_cur_rfs + rf_x)
                        bk = 0
                        if row >= self.DRAM_row:
                            row %= self.DRAM_row
                            bk = 1
